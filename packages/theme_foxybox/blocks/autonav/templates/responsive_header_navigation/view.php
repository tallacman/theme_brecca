...
if ($navItems !== []) {
    echo '<ul id="menu">'; //opens the top-level menu

    foreach ($navItems as $ni) {

        echo '<li class="' . h($ni->classes) . '">'; //opens a nav item
        echo '<a href="' . h($ni->url) . '" target="' . h($ni->target) . '" class="' . h($ni->classes) . '">' . $ni->name . '</a>';

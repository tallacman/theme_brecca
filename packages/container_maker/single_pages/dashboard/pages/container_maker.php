...
<div class="cm-import-wrap card mb-3" id="cmImportSection">
    ...
    <div class="card-body py-3">
        <ul class="nav nav-tabs nav-tabs-sm mb-2" role="tablist">
            <li class="nav-item"><button type="button" class="nav-link active" data-cm-import-tab="theme"><?= t('Installed') ?></button></li>
            <li class="nav-item"><button type="button" class="nav-link" data-cm-import-tab="upload"><?= t('Upload') ?></button></li>
            <li class="nav-item"><button type="button" class="nav-link" data-cm-import-tab="paste"><?= t('Paste') ?></button></li>
        </ul>
        <div id="cmImportTabTheme" data-cm-import-panel="theme">
        ...
        <div id="cmImportTabUpload" data-cm-import-panel="upload" class="d-none">
            <form method="post" action="<?= $view->action('import') ?>" enctype="multipart/form-data" class="cm-import-panel">
                <?php $token->output('container_maker_import'); ?>
                <input type="hidden" name="themeID" value="<?= (int) $selectedThemeID ?>">
                <input type="hidden" name="importSource" value="upload">
                <div class="row g-2 align-items-end">
                    <div class="col-md-4">
                        <label class="form-label mb-1"><?= t('Container handle') ?></label>
                        <input type="text" name="importHandle" class="form-control form-control-sm" placeholder="<?= h(t('Optional')) ?>">
                    </div>
                    <div class="col-md-5">
                        <label class="form-label mb-1"><?= t('PHP file') ?></label>
                        <input type="file" name="importFile" class="form-control form-control-sm" accept=".php,text/x-php">
                    </div>
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-outline-primary btn-sm w-100"><?= t('Upload & edit') ?></button>
                    </div>
                </div>
            </form>
        </div>
        <div id="cmImportTabPaste" data-cm-import-panel="paste" class="d-none">
            <form method="post" action="<?= $view->action('import') ?>" class="cm-import-panel">
                <?php $token->output('container_maker_import'); ?>
                <input type="hidden" name="themeID" value="<?= (int) $selectedThemeID ?>">
                <input type="hidden" name="importSource" value="paste">
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label mb-1"><?= t('Container handle') ?></label>
                        <input type="text" name="importHandle" class="form-control form-control-sm" placeholder="<?= h(t('Optional')) ?>">
                    ...
                    <div class="col-md-8">
                        <label class="form-label mb-1"><?= t('Container PHP') ?></label>
                        <textarea name="importPaste" class="form-control form-control-sm" rows="4" placeholder="<?= h(t('Paste the container PHP markup')) ?>"></textarea>
                    </div>
                    <div class="col-12 text-end">
                        <button type="submit" class="btn btn-outline-primary btn-sm"><?= t('Paste & edit') ?></button>
                    </div>
                </div>
            </form>
        </div>
...
<form method="post" action="<?= $view->action('save') ?>" id="cmForm" class="cm-app">
    ...
    <input type="hidden" name="area_ids" id="cmAreaIds" value="">
    ...
    <div class="cm-workspace">
        <aside class="cm-palette">
            ...
            <details class="mt-2" id="cmAreaNamesWrap" open>
                <summary class="cm-panel-title mb-0"><?= t('Area names') ?></summary>
                <div id="cmAreaNames" class="cm-area-names mt-2"></div>
            </details>
        ...
        <section class="cm-main">
            ...
            <div id="cmSelection" class="cm-selection d-none">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-3">
                        <label class="form-label mb-1"><?= t('Area ID') ?></label>
                        <input type="text" id="cmSelId" class="form-control form-control-sm" placeholder="<?= h(t('Optional')) ?>">
                    </div>
                    <div class="col-md-2">
    ...
    <div class="cm-toolbar">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-lg-2">
                <label class="form-label" for="cmContainerId"><?= t('Container ID') ?></label>
                <input type="text" name="container_id" id="cmContainerId" class="form-control form-control-sm" placeholder="<?= h(t('Optional')) ?>">
            </div>
...
.cm-workspace{display:flex;flex-wrap:wrap;gap:1rem;min-height:460px}
...
.cm-palette{flex:0 1 260px}
.cm-main{flex:1 1 560px}
.cm-code-wrap{flex:0 1 320px}
...
.cm-area-names{display:grid;gap:.35rem}
...
@media (max-width:1199px){.cm-palette,.cm-main,.cm-code-wrap{flex:1 1 100%}.cm-code-wrap{order:3}}
</style>
...
(function(){
    ...
    const ROW_H = 72;
    const PRESETS = [
        { id:'free', label:'<?= h(t('Free block')) ?>', areas:[{name:'Block',span:6}], hint:'<?= h(t('One area — drag block to place, use area settings for width')) ?>' },
    ...
    function cleanId(v){
        v = String(v || '').trim().replace(/[^A-Za-z0-9\-_:.]+/g, '-').replace(/^-+|-+$/g, '');
        if (!v) return '';
        return /^[A-Za-z]/.test(v) ? v : ('cm-' + v);
    }
    function defaultArea(name,col,span,row){
        row = row || 1;
        return {name, id:'', col:col||1, span:span||6, row, rowSpan:1, full:false, padding:'', gridColumn:'', gridRow:'', minHeight:'', tabletSpan:null, mobileSpan:null, mobileStack:true, tabletCol:null, mobileCol:null, tabletRow:row, mobileRow:row};
    }
    ...
    function renderBootstrapCode(){
        ...
        const containerId = cleanId($('cmContainerId')?.value || '');
        ...
        const body = '<div class="'+cls+'"'+idAttr+'>\n'+indent(wrapped,4)+'\n</div>';
    ...
    function renderAreaNames(){
        const wrap = $('cmAreaNames');
        if (!wrap) return;
        if (!areas.length) {
            wrap.innerHTML = '<div class="small text-muted"><?= h(t('Add a preset, then rename areas here.')) ?></div>';
            return;
        }
        wrap.innerHTML = areas.map((a, i)=>(
            '<div class="input-group input-group-sm">'
            +'<span class="input-group-text">'+(i+1)+'</span>'
            +'<input type="text" class="form-control" data-area-name-index="'+i+'" value="'+esc(a.name)+'">'
            +'</div>'
        )).join('');
    }
    ...
    function renderTiles(){
        ...
        $('cmTiles').innerHTML = areas.map((a,i)=>{
            ...
            const idAttr = a.id ? ' id="'+esc(cleanId(a.id))+'"' : '';
            return '<div class="cm-tile'+(i===selected?' active':'')+'" data-i="'+i+'" style="'+style+'"'+idAttr+'>'
    ...
    function syncHidden(){
        ...
        $('cmPadding').value = '';
        if ($('cmAreaIds')) $('cmAreaIds').value = areas.filter(a=>a.id).map(a=>a.name+'::'+cleanId(a.id)).join('\n');
    }
    ...
    function renderCode(){
        ...
        const items = areas.map(a=>{
            const idAttr = a.id ? ' id="'+cleanId(a.id)+'"' : '';
            return '<div class="'+cls+'__item"'+idAttr+'>\n'+indent(areaBlock(a.name),4)+'\n</div>';
        }).join('\n');
        ...
        rules.push('.'+cls+'__grid { display: grid; grid-template-columns: '+buildGridColumnsCss()+'; grid-auto-rows: '+buildGridRowsCss()+'; gap: var(--cm-theme-gap, 1rem); }');
        areas.forEach((a,i)=>{
            const sel = '.'+cls+'__grid > *:nth-child('+(i+1)+')';
    ...
    function loadEdit(ed){
        ...
        if(ed.settings){
            ...
            if(ed.settings.container_id) $('cmContainerId').value = ed.settings.container_id;
    ...
    $('cmAreaNames')?.addEventListener('input', e=>{
        const input = e.target.closest('[data-area-name-index]');
        if (!input) return;
        const i = parseInt(input.dataset.areaNameIndex, 10);
        if (Number.isNaN(i) || !areas[i]) return;
        areas[i].name = input.value;
        if (selected === i && $('cmSelName')) $('cmSelName').value = input.value;
        const tile = document.querySelector('.cm-tile[data-i="'+i+'"] .cm-tile-name');
        if (tile) tile.textContent = input.value;
        syncHidden();
        renderCode();
    });
    ...
    document.addEventListener('pointerup', ()=>{
        ...
        if (drag.moved) window.scrollTo(scrollX, scrollY);
        drag = null;
    });

    // Selection panel
    $('cmSelName').addEventListener('input', e=>{ if(areas[selected]){ areas[selected].name=e.target.value; renderTiles(); }});
    $('cmSelId').addEventListener('input', e=>{ if(areas[selected]){ areas[selected].id=cleanId(e.target.value); renderTiles(); }});
    ...
    ['cmName','cmHandle','cmInner','cmContainerId','cmSideMargin','cmRowMode','cmRowValue','cmTabletColumns','cmMobileColumns','cmBootstrapGutter'].forEach(id=>{
    ...
    document.querySelectorAll('[data-cm-import-tab]').forEach(btn=>{
        btn.addEventListener('click', ()=>{
            const tab = btn.dataset.cmImportTab || 'theme';
            document.querySelectorAll('[data-cm-import-tab]').forEach(b=>b.classList.toggle('active', b === btn));
            document.querySelectorAll('[data-cm-import-panel]').forEach(panel=>{
                panel.classList.toggle('d-none', panel.dataset.cmImportPanel !== tab);
            });
        });
    });

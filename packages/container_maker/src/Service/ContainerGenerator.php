...
class ContainerGenerator
{
    ...
    public function generate(Theme $theme, array $data)
    {
        ...
        if (file_exists($file) && !$overwrite) {
            ...
            if ($customCode !== null) {
                ...
                $php = $this->buildBootstrapContainerPhp($handle, $name, $areas, $settings, $areaFullRows, $responsiveColumns, $nestedAreas, $areaPadding, $areaPlacement, $areaResponsive);
            } else {
    ...
    protected function buildContainerPhp($handle, $name, array $areas, array $settings, array $areaFullRows, array $responsiveColumns, array $nestedAreas, array $areaPadding, array $areaPlacement = [], array $areaResponsive = [])
    {
        ...
        $designerState = $this->encodeDesignerState($handle, $name, 'css_grid', $settings, $areas, $responsiveColumns, $areaFullRows, $areaPadding, $areaPlacement, $nestedAreas, $areaResponsive);
    ...
    protected function buildBootstrapContainerPhp($handle, $name, array $areas, array $settings, array $areaFullRows, array $responsiveColumns, array $nestedAreas, array $areaPadding, array $areaPlacement = [], array $areaResponsive = [])
    {
        $class = 'cm-' . str_replace('_', '-', $handle);
        $inner = $settings['inner'];
        $designerState = $this->encodeDesignerState($handle, $name, 'bootstrap5', $settings, $areas, $responsiveColumns, $areaFullRows, $areaPadding, $areaPlacement, $nestedAreas, $areaResponsive);

        [$body, $css] = $this->buildBootstrapMarkup($class, $inner, $areas, $areaFullRows, $responsiveColumns, $settings, $areaPadding, $areaPlacement, $areaResponsive);
        $headerPhp = $this->buildHeaderStyleInjection($class, $css);

        return $this->wrapContainerPhp($designerState, $headerPhp, $body);
    }

    protected function encodeDesignerState($handle, $name, $layoutMode, array $settings, array $areas, array $responsiveColumns, array $areaFullRows, array $areaPadding, array $areaPlacement, array $nestedAreas, array $areaResponsive)
    {
...
PHP;
    ...
    protected function buildCssGridMarkup($class, $inner, array $areas, array $areaFullRows, array $responsiveColumns, array $settings, array $areaPadding = [], array $areaPlacement = [], array $areaResponsive = [])
    {
        ...
        $rules = array_merge(
            $this->buildCssGridRootRules($class, $settings),
            [
    ...
    protected function buildCssGridRootRules($class, array $settings)
    {
        $rules = [];
        $sideMargin = trim((string) ($settings['side_margin'] ?? ''));
        if ($sideMargin !== '') {
            $rules[] = '.' . $class . ' { margin-left: ' . $sideMargin . '; margin-right: ' . $sideMargin . '; }';
        }

        return $rules;
    }

    protected function buildBootstrapMarkup($class, $inner, array $areas, array $areaFullRows, array $responsiveColumns, array $settings, array $areaPadding = [], array $areaPlacement = [], array $areaResponsive = [])
    {
        ...
        foreach ($rowKeys as $rowIndex => $rowNum) {
            ...
            foreach ($rowAreas as $areaName) {
                ...
                $style = '';
                if (!empty($areaPadding[$areaName])) {
                    $style = ' style="padding: ' . $areaPadding[$areaName] . ';"';
                }
                ...
                $cols[] = '<div class="' . $itemClass . '"' . $style . ">\n" . $this->indentLines($this->areaPhpBlock($areaName), 4) . "\n</div>";
            }
        }
    ...
    protected function buildBootstrapCssBundle($class, array $areas, array $areaFullRows, array $responsiveColumns, array $settings, array $areaPlacement, array $areaResponsive, array $areaPadding)
    {
        $rules = [];
        if (empty($settings['theme_has_bootstrap'])) {
            $rules[] = $this->buildBootstrapScopedGridCss($class, $settings['bootstrap_gutter'] ?? 'g-3');
        }
        $rules[] = $this->buildBootstrapOrderCss($class, $areas, $areaPlacement, $areaResponsive);
        $rules[] = '.ccm-edit-mode .' . $class . ' .ccm-area { min-height: 48px; outline: 1px dashed rgba(0, 0, 0, .25); outline-offset: -1px; }';

        return implode("\n", array_filter($rules));
    }

    protected function buildBootstrapOrderCss($class, array $areas, array $areaPlacement, array $areaResponsive)
    {
        $tabletRules = [];
        $mobileRules = [];
        $itemIndex = 0;
        foreach ($areas as $areaName) {
            ...
            $selector = '.' . $class . '__item--' . $itemIndex;
        ...
        if (!empty($tabletRules)) {
            $css .= "@media (max-width: 991px) {\n    ." . $class . " .row { display: flex; flex-wrap: wrap; }\n    ." . $class . "__row-break { display: none !important; height: 0; flex-basis: 0; }\n    " . implode("\n    ", $tabletRules) . "\n}\n";
        }
    ...
    protected function buildBootstrapScopedGridCss($class, $gutter)
    {
    ...
    protected function normalizeSettings(array $data)
    {
        ...
        return [
            ...
            'gap' => $layoutMode === 'bootstrap5'
                ? ''
                : $this->safeCssLength($data['gap'] ?? '1rem', '1rem'),
